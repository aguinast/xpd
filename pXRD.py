...
class Defaults():
    ...
    def createDefault(self):
        ...
        # 2th range
        list = ["# 2th range", "tth_min = 5", "tth_max = 65", "tth_step = 0.01"]
        for i in list: lstout.append(i)
        # hkl max
        list = ["# hkl max","h_max = 6","k_max = 6","l_max = 6"]
        for i in list: lstout.append(i)
        # initial energy and energy range
        list = ["# energy", "E_ini = 8", "E_min = 4", "E_max = 20"]
        for i in list: lstout.append(i)
        # initial crystal size and crystal size range
        list = ["# crystal size", "CrysSize_ini = 500", "CrysSize_min = 15", "CrysSize_max = 1000"]
        for i in list: lstout.append(i)
        # initial lattice parameters
        list = ["# initial lattice parameters", "a_ini = 5.640", "b_ini = 5.640", "c_ini = 5.640", "alpha_ini = 90", "beta_ini = 90", "gamma_ini = 90"]
        for i in list: lstout.append(i)
        # initial lattice parameters minumum
        list = ["# initial lattice parameters minimum", "a_min = 3.000", "b_min = 3.000", "c_min = 3.000", "alpha_min = 40", "beta_min = 40", "gamma_min = 40"]
        for i in list: lstout.append(i)
        # initial lattice parameters maximum
        list = ["# initial lattice parameters maximum", "a_max = 13.000", "b_max = 13.000", "c_max = 13.000", "alpha_max = 150", "beta_max = 150", "gamma_max = 150"]
        for i in list: lstout.append(i)
        # initial base atom
        list = ["# initial base atom", "atom_ini = Fe" ]
        for i in list: lstout.append(i)
        # language
        list = ["# initial language", "version = en"]
        for i in list: lstout.append(i)
        # micelaneous
        list = ["# other options", "font_ini = Currier", "fontsize_ini = 11"]
        for i in list: lstout.append(i)
...
class Freeze(ToolToggleBase):
    ...
    def __init__(self, *args, curve, plot, main_plot, **kwargs):
        ...
        self.description = names[inis['version']]['freeze0'] + ' (' + names[inis['version']]['freeze1'].format(names[inis['version']][self.curve_color[curve]]) + ')'
...
class Window(QMainWindow,QWidget):
    def __init__(self, initial_values, parent=None):
        ...
        self.initial_parameters(initial_values)
    ...
    def initial_parameters(self, initial_values):
        # importing defaults
        inis = initial_values
        self.version = inis['version']
        self.font = inis['font_ini']
        self.fontsize = int(float(inis['fontsize_ini']))
        ...
        self.par0 = {'a': float(inis['a_ini']),  'b': float(inis['b_ini']),  'c':float(inis['c_ini']),  'alpha':float(inis['alpha_ini']),  'beta':float(inis['beta_ini']),  'gamma':float(inis['gamma_ini'])}
        self.par_min =  {'a': float(inis['a_min']),  'b': float(inis['b_min']),  'c':float(inis['c_min']),  'alpha':float(inis['alpha_min']),  'beta':float(inis['beta_min']),  'gamma':float(inis['gamma_min'])}
        self.par_max =  {'a': float(inis['a_max']),  'b': float(inis['b_max']),  'c':float(inis['c_max']),  'alpha':float(inis['alpha_max']),  'beta':float(inis['beta_max']),  'gamma':float(inis['gamma_max'])}

        # initial energy
        self.init_E = float(inis['E_ini'])
        self.E = self.init_E
        xu.energy(self.init_E*1000)
        self.E_min = float(inis['E_min'])
        self.E_max = float(inis['E_max'])

        # initial size
        self.init_D = float(inis['CrysSize_ini'])
        self.D = self.init_D
        self.D_min = float(inis['CrysSize_min'])
        self.D_max = float(inis['CrysSize_max'])

        # initial atom
        self.initial_atom = inis['atom_ini']
    ...
    def eventFilter(self, source, event, flag = 0):
        if flag == 0:
            if event.type() == QEvent.Enter and source is self.XPDFigure_groupBox: self.Info_label.setText(names[self.version]['xpd'])
        elif flag in self.LatticeParams:
            if event.type() == QEvent.Enter and source is self.LatticeParams_button[flag]: self.Info_label.setText(names[self.version]['lattpar'])
            elif event.type() == QEvent.Enter and source is self.LatticeParams_slider[flag]: self.Info_label.setText(names[self.version]['lattpar'])
            elif event.type() == QEvent.Enter and source is self.LatticeParams_entry[flag]: self.Info_label.setText(names[self.version]['lattpar'])
        #elif 
    ...
    def include_XPDFigure(self):
        ...
        self.XPDFigure_groupBox = QGroupBox(names[self.version]['title'], self)
        self.XPDFigure_groupBox.setFont(QFont(self.font,self.fontsize))
    ...
    def include_HKL(self):
        ...
        self.le_H.setToolTip(names[self.version]['hkl_inf'] + names[self.version]['hkl_inf2'])
        self.le_K.setToolTip(names[self.version]['hkl_inf'] + names[self.version]['hkl_inf2'])
        self.le_L.setToolTip(names[self.version]['hkl_inf'] + names[self.version]['hkl_inf2'])
        ...
        self.showHKL_check.setToolTip(names[self.version]['showHKL'])
    ...
    def update_HKL_H(self):
        ...
        try:
            ...
            if val_ <-9 or val_ > 9:
                ...
                text = '"{}" {} {}'.format(val_, names[self.version]['problem_a'], 'H')
                ...
                text = '{}'.format(names[self.version]['hkl_000'])
                self.Update_Info_label(text = text, bkg = self.red)
                self.le_H.setText(str(self.old_HKL_H))
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'], 'H')
            self.Update_Info_label(text = text, bkg = self.red)
            self.le_H.setText(str(self.old_HKL_H))
    def update_HKL_K(self):
        ...
        try:
            val_ = int(val)
            if val_ <-9 or val_ > 9:
                text = '"{}" {} {}'.format(val_, names[self.version]['problem_a'], 'K')
                self.Update_Info_label(text = text, bkg = self.red)
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'], 'K')
            self.Update_Info_label(text = text, bkg = self.red)
            self.le_K.setText(str(self.old_HKL_K))
    def update_HKL_L(self):
        ...
        try:
            val_ = int(val)
            if val_ <-9 or val_ > 9:
                text = '"{}" {} {}'.format(val_, names[self.version]['problem_a'], 'L')
                self.Update_Info_label(text = text, bkg = self.red)
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'],'L')
            self.Update_Info_label(text = text, bkg = self.red)
            self.le_L.setText(str(self.old_HKL_L))
    ...
    def include_LatticeParams(self):
        # just the name of the group
        self.LatticeParams_groupBox = QGroupBox(names[self.version]['pars'], self)
        ...
        for i in self. LatticeParams:
            ...
            self.LatticeParams_button[i].setToolTip(self.par_name[i] + ': ' + names[self.version]['tt_b_pars'])
            if i in ['a', 'b', 'c']:
            ...
            self. LatticeParams_slider[i].setToolTip(names[self.version]['tt_s_pars'])
            ...
            self.LatticeParams_entry[i].setToolTip(names[self.version]['tt_e_pars'])
    ...
    def update_sliders(self, par):
        ...
        try:
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'], names[self.version][par])
            self.Update_Info_label(text = text, bkg = self.red)
            self.LatticeParams_entry[par].setText(str(self.LatticeParams_slider[par].value()))

    def include_CrystalFigure(self):
        # just the name of the group
        self.CrystalFigure_groupBox = QGroupBox(names[self.version]['crystal'], self) 
        ...
        self.Vis_slider.setToolTip(names[self.version]['tt_s_uc'])
        ...
        self.Edge_check.setToolTip(names[self.version]['crysedge'])
        ...
        self.Face_check.setToolTip(names[self.version]['crysface'])
        ...
        self.Atoms_check.setToolTip(names[self.version]['crysatoms'])
    ...
    def include_E(self):
        # just the name of the group
        self.E_groupBox = QGroupBox(names[self.version]['wvl'], self)
        ...
        self.E_button.setToolTip(names[self.version]['en_0'] + ': ' + names[self.version]['tt_b_pars'])
        
        self.Wvl_button = QPushButton('\u03bb (\u212b)')
        self.Wvl_button.clicked.connect(self.reset_E)
        self.Wvl_button.setStyleSheet(button_opts)
        self.Wvl_button.setToolTip(names[self.version]['wvl_0'] + ': ' + names[self.version]['tt_b_pars'])
        ...
        self.E_slider.setToolTip(names[self.version]['tt_s_pars'])
        ...
        self.Wvl_slider.setToolTip(names[self.version]['tt_s_pars'])
        ...
        self.E_entry.setToolTip(names[self.version]['tt_e_pars'])
        ...
        self.Wvl_entry.setToolTip(names[self.version]['tt_e_pars'])
    ...
    def E_update_slider(self):
        ...
        try:
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'], names[self.version]['en_0'])
            self.Update_Info_label(text = text, bkg = self.red)
            self.E_entry.setText('{: 5.2f}'.format(self.E))
    def Wvl_upadate_slider(self):
        ...
        try:
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'], names[self.version]['wvl_0'])
            self.Update_Info_label(text = text, bkg = self.red)
            self.Wvl_entry.setText('{: 5.2f}'.format(12.398/self.E))
    def updateQs(self):
    ...
    def include_CrystalSize(self):
        # just the name of the group
        self.CrystalSize_groupBox = QGroupBox(names[self.version]['size'], self) 
        ...
        self.CrystalSize_button.setToolTip(names[self.version]['size_0'] + ': ' + names[self.version]['tt_b_pars'])
        ...
        self.CrystalSize_slider.setToolTip(names[self.version]['tt_s_pars'])
        ...
        self.CrystalSize_entry.setToolTip(names[self.version]['tt_e_pars'])
    ...
    def Size_update_slider(self):
        ...
        try:
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'], names[self.version]['size_0'])
            self.Update_Info_label(text = text, bkg = self.red)
            self.CrystalSize_entry.setText('{: 6.1f}'.format(self.D))

    def include_InfoFrame(self):
        self.Info_groupBox = QGroupBox(names[self.version]['info'], self) 
        ...
        self.Info_label.setText(names[self.version]['welcome'])
    ...
    def include_BaseAtoms(self):
        # just the name of the group
        self.BaseAtoms_groupBox = QGroupBox(names[self.version]['base'], self)
        ...
        self.BaseAtoms_entry_0.setToolTip(names[self.version]['label_0'])
        ...
        self.BaseAtoms_color.setToolTip(names[self.version]['color_0'])

        self.BaseAtoms_button_addAtom = QPushButton('+ {}'.format(names[self.version]['atm']))
        ...
        self.BaseAtoms_button_addAtom.setToolTip(names[self.version]['addAtoms'])
        self.BaseAtoms_button_remAtom = QPushButton('- {}'.format(names[self.version]['atm']))
        ...
        self.BaseAtoms_button_remAtom.setToolTip(names[self.version]['remAtoms'])
    ...
    def newAtomEntered(self, atom):
        ...
        try:
            ...
            text = '"{}" {} {}'.format(text0, names[self.version]['problem_a'], names[self.version]['atm'])
            self.Update_Info_label(text = text, bkg = self.red)
    def add_Atom(self):
    ...
    def include_atom(self):
        ...
        self.AddAtoms_groupBox.update({i:QGroupBox('{} {}'.format(names[self.version]['atm'],i))})
        self.AddAtoms_groupBox[i].setFont(QFont(self.font,self.fontsize))
        self.AddAtoms_groupBox[i].setStyleSheet(self.GroupBox_StyleSheet(self.colors[i-1],'#dddddd'))
        ...
        self.AddAtoms_slider_x[i].setToolTip(names[self.version]['atm'] + '{}, '.format(i) + names[self.version]['posx'] + ': '.format(i) + names[self.version]['tt_s_pars'])
        ...
        entry_opts = self.LineEdit_StyleSheet(self.colors[i-1])
        ...
        self.AddAtoms_entry_pos_x[i].setToolTip(names[self.version]['atm'] + '{}, '.format(i) + names[self.version]['posx'] + ': '.format(i) + names[self.version]['tt_e_pars'])
        ...
        self.AddAtoms_slider_y[i].setToolTip(names[self.version]['atm'] + '{}, '.format(i) + names[self.version]['posy'] + ': '.format(i) + names[self.version]['tt_s_pars'])
        ...
        self.AddAtoms_entry_pos_y[i].setToolTip(names[self.version]['atm'] + '{}, '.format(i) + names[self.version]['posy'] + ': '.format(i) + names[self.version]['tt_e_pars'])
        ...
        self.AddAtoms_slider_z[i].setToolTip(names[self.version]['atm'] + '{}, '.format(i) + names[self.version]['posz'] + ': '.format(i) + names[self.version]['tt_s_pars'])
        ...
        self.AddAtoms_entry_pos_z[i].setToolTip(names[self.version]['atm'] + '{}, '.format(i) + names[self.version]['posz'] + ': '.format(i) + names[self.version]['tt_e_pars'])
        ...
        self.AddAtoms_entry_At[i].setToolTip(names[self.version]['atomtype'])
        ...
        button_opts = self.PushButton_StyleSheet2(self.colors[i-1])
        self.AddAtoms_color[i].setStyleSheet(button_opts)
        self.AddAtoms_color[i].setToolTip(names[self.version]['color_0'])
    ...
    def new_pos_x(self,atom):
        ...
        try:
            ...
            text = '"{}" {} {}'.format(val, names[self.version]['problem_a'], names[self.version]['atm_0'])
            self.Update_Info_label(text = text, bkg = self.red)
            self.AddAtoms_entry_pos_x[atom].setText('{: 5.3f}'.format(self.AddAtoms_slider_x[atom].value()))
    ...
    def calculate_F2(self,hkl,en_, Q):
        ...
        if False:
            ...
            for i in range (self.additional_atoms):
                 ...
                 pos = [self.AddAtoms_slider_x[i+1].value(),self.AddAtoms_slider_y[i+1].value(),self.AddAtoms_slider_z[i+1].value()]
                 Fhkl += f*np.exp(-2*np.pi*1j*np.dot(hkl,pos))
        return pow(np.abs(Fhkl),2)

    def Q2tth(self,Q, wvl): return 360./np.pi*np.arcsin(Q*wvl/(4*np.pi))
    def size_width(self,tth, wvl, CrysSize): return 0.9*wvl/(2.355*CrysSize*np.cos(tth/2.*self.degree))
    def calculate_intensity(self,hkl, wvl, Q, CrysSize): 
        tth = self.Q2tth(Q, wvl)
        w = self.size_width(tth, wvl, CrysSize)*360/np.pi
        return 1./(np.sqrt(2*np.pi)*w)*np.exp(-(self.theta_range-tth)*(self.theta_range-tth)/(2.*w*w))

if __name__ == '__main__':
    default = Defaults()
    if not os.path.isfile("pxrd.defaults"): default.createDefault()
    inis = default.loadDefault()
